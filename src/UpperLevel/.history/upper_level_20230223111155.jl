...
@constraint(model, obj1, objective_terms[1] >= K_l * sum(alpha[k, l] * line_cost[k][l] * line_length[l] for k in 1:K, l in 1:L))
@constraint(model, obj2, objective_terms[2] >= K_s * sum(beta[i] * substation_cost[i] for i in 1:n_s))
@constraint(model, obj3, objective_terms[3] >= HOURS_IN_A_YEAR * (1 + interest_rate_losses) * loss_cost * line_loss * sum((P_s[i] - P_demand[i]) for i in 1:N) * BASE_POWER)
@constraint(model, obj4, objective_terms[4] >= HOURS_IN_A_YEAR * (1 + interest_rate_substation) * substation_utilization * sum(substation_operation_cost[i]*(P_s[i]^2 + Q_s[i]^2) for i in 1:n_s)* BASE_POWER^2)
