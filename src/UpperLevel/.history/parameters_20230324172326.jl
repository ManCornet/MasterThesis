...
end_date   = DateTime(start_date + Year(nb_years))

period  = 24 # nb of 5min. time steps to aggregate
delta_t = GRANULARITY * period # [min]

# Period covered
T = start_date:Minute(delta_t):(end_date-Second(1))
nb_years = length(unique(year.(T))) # nb of years in the planning horizon
nb_days  = length(unique(day.(T)))
