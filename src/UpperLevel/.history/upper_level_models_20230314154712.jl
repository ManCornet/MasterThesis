...
function MISOCP_formulation(N, n_s, n_s_init, L, K ,
   ...
   @objective(model, Min,  K_l              * sum((alpha[k, l] * line_cost[k][l] 
    ...
    @constraint(model, 
                losses_constraint3[k=1:K, l=1:L],
                losses[k, l] == I_squared[k, l]/conductance[k][l])
                )
    @constraint(model, 
                losses_constraint1,
    ...
    @constraint(model, 
                ...
                P_s[i] - P_demand[i] ==   sum(P_conductor[k, l] - losses[k, l]
                                            for k in 1:K, l in Omega_sending[i])
                                        - sum(P_conductor[k, l] - losses[k, l]
