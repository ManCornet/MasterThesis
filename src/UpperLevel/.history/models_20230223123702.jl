...
function MISOCP_formulation( N, n_s, L, K ,
    ...
    @constraint(model, 
                obj_term1, 
                epigraphs[1] >= K_l * sum(alpha[k, l] * line_cost[k][l] * line_length[l] for k in 1:K, l in 1:L)
                )
          
    @constraint(model, epigraphs[2] >= )
    @constraint(model, epigraphs[3] >= )
    @constraint(model, epigraphs[4] >= )
