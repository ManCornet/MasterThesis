...
function MINLP_formulation( N, n_s, L, K ,
                            ...
                            cost_unit_loss, Omega_sending, Omega_receiving, 
                            P_demand, Q_demand
                            )                      
    ...
    if termination_status(model) == MOI.OPTIMAL
        ...
        P_s         = value.(P_s)
        Q_s         = value.(Q_s)
        P_conductor = transpose(value.(P_conductor))
        Q_conductor = transpose(value.(Q_conductor))
        obj         = objective_value(model)
        time        = solve_time(model)
        #=
        println("")
        println("objective  ",Int(round(objective_value(model))))
        println("")
        println("x            ",value.(x))
        println("I_squared    ",value.(I_squared))
        println("P_conductor  ",value.(P_conductor))
        println("Q_conductor  ",value.(Q_conductor))
        println("")
        println("beta      ",value.(beta))
        println("alpha      ",value.(alpha))
        println("P_s       ",value.(P_s))
        println("Q_s       ",value.(Q_s))
        println("V_re      ",value.(V_re))
        println("V_im      ",value.(V_im))
        =#
...
function MISOCP_formulation( N, n_s, L, K ,
    ...
    @objective(model, Min,  K_l * sum((alpha[k, l] * line_cost[k][l] * line_length[l]) for k in 1:K, l in 1:L) + K_s* sum(beta[i] * substation_fixed_cost[i] for i in 1:n_s) + HOURS_IN_A_YEAR * (1 + interest_rate_losses)* line_loss * cost_unit_loss * sum((P_s[i] - P_demand[i]) for i in 1:N) * BASE_POWER
                          +  HOURS_IN_A_YEAR * (1 + interest_rate_substation) * substation_utilization * sum(substation_op_cost[i]* (P_s[i]^2 + Q_s[i]^2) for i in 1:n_s) * BASE_POWER^2
                )
    ...
    for k in 1:K, l in 1:L
        ...
        # CONSTRAINT (24)
        @constraint(model,   MIN_VOLTAGE^2 * alpha[k, l] 
                          <= X_i_ij[k, ifrom, l]
                    )
