...
function MISOCP_formulation(N, n_s, L, K ,
    ...
    for k = 1:K, l = 1:L
        ...
        # CONSTRAINT (24)
        @constraint(model, MIN_VOLTAGE * MIN_VOLTAGE * alpha[k, l] <= X_i_ij[k, ifrom, l])
        @constraint(model, MAX_VOLTAGE * MAX_VOLTAGE * alpha[k, l] >= X_i_ij[k, ifrom, l])
        @constraint(model, MIN_VOLTAGE * MIN_VOLTAGE * alpha[k, l] <= X_i_ij[k, ito, l])
        @constraint(model, MAX_VOLTAGE * MAX_VOLTAGE * alpha[k, l] >= X_i_ij[k, ito, l])

        # CONSTRAINT (27)
        @constraint(model,  MIN_VOLTAGE^2*(1-alpha[k, l] <= (V_re[ifrom]^2 + V_im[i_from]^2)
    ...
    # CONSTRAINT (25)
    @constraint(model, [k=1:K, l=1:L], X_ij_re[k, l] <= MAX_VOLTAGE*MAX_VOLTAGE * alpha[k, l])

    # CONSTRAINT (26)
    @constraint(model, [k=1:K, l=1:L], X_ij_im[k, l] <= MAX_VOLTAGE*MAX_VOLTAGE * alpha[k, l])
    @constraint(model, [k=1:K, l=1:L], X_ij_im[k, l] >= -MAX_VOLTAGE*MAX_VOLTAGE * alpha[k, l])
