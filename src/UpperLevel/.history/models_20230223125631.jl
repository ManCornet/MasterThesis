...
function MISOCP_formulation( N, n_s, L, K ,
    ...
    #@constraint(model, objective_terms[2] >= K_s * sum(beta[i] * substation_fixed_cost[i] for i in 1:n_s))
    #@constraint(model, objective_terms[3] >= HOURS_IN_A_YEAR * (1 + interest_rate_losses) * cost_unit_loss * line_loss * sum((P_s[i] - P_demand[i]) for i in 1:N) * BASE_POWER)
    #@constraint(model, objective_terms[4] >= HOURS_IN_A_YEAR * (1 + interest_rate_substation) * substation_utilization * sum(substation_op_cost[i]*(P_s[i]^2 + Q_s[i]^2) for i in 1:n_s)* BASE_POWER^2)
 
    @objective(model, Min, + epigraph)
    #@objective(model, Min, sum(epigraphs))
    #=
    @constraint(model, 
                obj_term1, 
                epigraphs[1] >= (K_l * sum((alpha[k, l] * line_cost[k][l] 
                                    * line_length[l]) for k in 1:K, l in 1:L))
                )
    ...
    @constraint(model, 
                obj_term2, 
                epigraphs[2] >= K_s * sum(beta[i] * substation_fixed_cost[i] for i in 1:n_s)
                )
    @constraint(model, 
                obj_term3, 
                epigraphs[3] >= (HOURS_IN_A_YEAR * (1 + interest_rate_losses) 
    ...
    @constraint(model, 
                obj_term4,
                epigraphs[4] >= (HOURS_IN_A_YEAR * (1 + interest_rate_substation) 
    ...
    @constraint(model, 
                active_balance[i=1:N], 
                P_s[i] - P_demand[i] == 
                (sum(P_conductor[k, l] for k in 1:K, l in Omega_sending[i])
    ...
    @constraint(model, 
                reactive_balance[i=1:N], 
                Q_s[i] - Q_demand[i] == 
                (sum(Q_conductor[k, l] for k in 1:K, l in Omega_sending[i])
    ...
    for k = 1:K, l = 1:L
        ...
        @constraint(model, 
                    P_conductor[k,l] == (
                       conductance[k][l] * (X_i_ij[k, ifrom, l] - X_ij_re[k, l]) 
        ...
        @constraint(model, 
                    Q_conductor[k,l] == (
                     - susceptance[k][l] * (X_i_ij[k, ifrom, l] - X_ij_re[k, l]) 
        ...
        @constraint(model, 
                    I_squared[k, l] == (   
                        (conductance[k][l]^2 + susceptance[k][l]^2) 
                      * (X_i_ij[k, ifrom, l] + X_i_ij[k, ito, l] - 2 * X_ij_re[k, l])
                    ))
        # CONSTRAINT (24)
        @constraint(model, MIN_VOLTAGE*MAX_VOLTAGE * alpha[k, l] <= X_i_ij[k, ifrom, l])
        @constraint(model, MAX_VOLTAGE^2 * alpha[k, l] >= X_i_ij[k, ifrom, l])
        @constraint(model, MIN_VOLTAGE*MAX_VOLTAGE * alpha[k, l] <= X_i_ij[k, ito, l])
        ...
        # CONSTRAINT (27)
        @constraint(model, V_squared[ifrom] - X_i_ij[k, ifrom, l] >= MIN_VOLTAGE^2 * (1 - alpha[k, l]))
        @constraint(model, V_squared[ifrom] - X_i_ij[k, ifrom, l] <= MAX_VOLTAGE^2 * (1 - alpha[k, l]))
        @constraint(model, V_squared[ito] - X_i_ij[k, ito, l] >= MIN_VOLTAGE^2 * (1 - alpha[k, l]))
        @constraint(model, V_squared[ito] - X_i_ij[k, ito, l] <= MAX_VOLTAGE^2 * (1 - alpha[k, l]))

        # CONSTRAINT (28)
        @constraint(model, 
                    [X_i_ij[k, ifrom, l]/2; X_i_ij[k, ito, l]; X_ij_re[k, l]; X_ij_im[k, l]] 
