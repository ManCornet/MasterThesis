...
function main()
    ...
    if formulation == 1
        ...
        print_table(convert(Array{Int64,2}, alpha), "Alpha" , "-", false) # 2D dimensions
        print_table(convert(Array{Int64,1}, x), "x", "-", false)
        print_table(convert(Array{Int64,1}, beta), "Beta", "-", false)
        @printf("-> Continuous variables: \n")
        print_table(I, "I" , "A", false)
        print_table(V .*BASE_VOLTAGE, "V", "kV", true)
        print_table(P_s .* BASE_POWER, "P_s", "kW", false)
        print_table(Q_s .* BASE_POWER, "Q_s", "kVar", false)
        print_table(P_conductor .* BASE_POWER, "P_conductor", "kW", false)
        print_table(Q_conductor .* BASE_POWER, "Q_conductor", "kVar", false)
