...
const ROOTDIR = @__DIR__ # fetches the current directory
...
function print_table(table, table_name, unity)
    ...
    end_string = 15 - length(table_name)
    @printf("%s", table_name)
...
function main()
    ...
    PROF_PATHS = joinpath(ROOTDIR, "Manchester_data/LCT_profiles")
    prof_files = readdir(PROF_PATHS, join=true)
    ...
    elseif formulation == 2
        ...
        @printf("-> Joule Losses (computation 1): %g \n\n", losses1)
        @printf("->  Joule Losses (computation 2): %g \n\n", losses2)
        @printf("-> Number of buses: %d \n\n", N)
        @printf("-> Number of lines: %d \n\n", L)
        @printf("-> Number of conductor types: %d \n\n", K)
        @printf("-> Binary variables: \n\n")
        ...
        print_table(abs.(P_s .* beta), "P_s", "pu")
        print_table(abs.(Q_s .* beta), "Q_s", "pu")
        print_2Dtable(abs.(P_cond_forward .* alpha), "P_cond_forward", "pu")
        print_2Dtable(abs.(P_cond_backward .* alpha), "P_cond_backward", "pu")
        print_2Dtable(abs.(Q_cond_forward .* alpha), "Q_cond_forward", "pu")
        print_2Dtable(abs.(Q_cond_backward .* alpha), "Q_cond_backward", "pu")
