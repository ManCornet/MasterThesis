...
function print_table(table, table_name, unity, type)
...
function print_2Dtable(table, table_name, unity, type)
...
function main()
    ...
    if formulation == 1
        ...
        int_alpha = convert(Array{UInt64,2}, alpha)
        int_beta  = convert(Array{UInt64,1}, beta)
        int_x     = convert(Array{UInt64,1}, x)
        
        @printf("-> Number of buses: %d \n\n", N)
        @printf("-> Number of lines: %d \n\n", L)
        @printf("-> Number of conductor types: %d \n\n", K)
        @printf("-> Binary variables: \n\n")
        print_2Dtable(int_alpha, "Alpha" , "-", "int") # 2D dimensions
        print_table(int_x, "x", "-", "int")
        print_table(int_beta, "Beta", "-", "int")
        @printf("\n-> Continuous variables: \n\n")
        print_table(I_squared .* int_alpha, "I" , "pu^2", "float")
        print_table(V, "V", "pu", "complex")
        print_table(P_s .* BASE_POWER .* int_beta, "P_s", "kW", "float")
        print_table(Q_s .* BASE_POWER .* int_beta, "Q_s", "kVar", "float")
        print_2Dtable(abs.(P_conductor .* int_beta .* BASE_POWER), "P_conductor", "kW", "float")
        print_2Dtable(abs.(Q_conductor .* int_alpha .* BASE_POWER), "Q_conductor", "kVar", "float")
    ...
    elseif formulation == 2
        ...
        @printf("-> Number of conductor types: %d \n\n", K)
        @printf("-> Binary variables: \n\n")
        print_2Dtable(alpha, "Alpha" , "-", "int") # 2D dimensions
        print_table(x, "x", "-", "int")
        print_table(beta, "Beta", "-", "int")
        @printf("\n-> Continuous variables: \n\n")
        print_table(I_squared .* int_alpha, "I_squared" , "pu^2", "float")
        print_table(V_squared, "V_squared", "pu^2", "complex")
        print_table(P_s .* BASE_POWER .* beta, "P_s", "kW", "float")
        print_table(Q_s .* BASE_POWER .* beta, "Q_s", "kVar", "float")
        print_2Dtable(abs.(P_conductor .* alpha .* BASE_POWER), "P_conductor", "kW", "float")
        print_2Dtable(abs.(Q_conductor .* alpha .* BASE_POWER), "Q_conductor", "kVar", "float")
        print_2Dtable(X, "X", "pu^2", "complex")
