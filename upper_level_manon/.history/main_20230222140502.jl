...
function main()
    ...
    substation_utilization   = 1        # phi_s : cost per energy lost [€/kWh]
    ...
    if formulation == 1
        ...
        println("I_squared  ", I_squared)
        println("x          ", x)
        println("alpha      ", alpha)
        println("beta       ", beta)
        println("P_s        ", P_s)
        println("Q_s        ", Q_s)
        println("P_conductor", P_conductor)
        println("Q_conductor", Q_conductor)
    ...
    elseif formulation == 2
        ...
        # LP formulation
        M          = [0 -40  0 0; 40 0 -60 80; 0 60 0 -20; 0 -80 20 0]
        method_num = 0
    
        I_squared, V_re, V_im, x, alpha, 
        beta, P_s, Q_s, P_conductor, Q_conductor, 
        obj, time  = MINLP_formulation( N, n_s, L, K ,
        K_l, K_s, 
        substation_utilization,
        S_rating_init, S_rating_max,
