...
@objective(model, Min, K_l * sum(alpha[k, l] * line_cost[k][l] for k in 1:K, l in 1:L)
                       +
                       ...
                       + HOURS_IN_A_YEAR * (1 + interest_rate_substation) * substation_utilization * sum((P_s[i]^2 + Q_s[i]^2) for i in 1:n_s)) * BASE_POWER
...
Q_cond_forward = transpose(value.(P_conductor_f))
Q_cond_backward = transpose(value.(P_conductor_b))
...
print_2Dtable(abs.(P_cond_backward .* alpha), "P_cond_forward", "kW")
print_2Dtable(abs.(Q_cond_forward .* alpha), "Q_cond_forward", "kVar")
print_2Dtable(abs.(Q_cond_backward .* alpha), "Q_cond_forward", "kVar")
