...
function MISOCP_formulation(N, n_s, L, K ,
    ...
    @variable(model, S_expansion[1:n_s]) 
    ...
    @objective(model, Min,  sum(alpha[k, l] * line_length[l] * line_cost[k][l] for k in 1:K, l in 1:L) 
                          + sum(beta[i] * substation_fixed_cost[i] for i in (n_s_init+1):n_s)
                          + sum(S_expansion[i] * substation_variable_cost[i] for i in 1:n_s) * BASE_POWER
                          + sum(((1 + infl)/(1 + d))^y * sum( for t in 1:T) for y in 1:Y)
    )
    ...
    Q_s[i] - Q_demand[i] == sum(Q_conductor[k, l] for k in 1:K, l in Omega_sending[i]) - sum(Q_conductor[k, l]
                        for k in 1:K, l in Omega_receiving[i]))
