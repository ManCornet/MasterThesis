...
function main()
    ...
    if formulation == 1
        ...
        int_alpha = convert(Array{Int64,2}, alpha)
        int_beta  = convert(Array{Int64,1}, beta)
        int_x     = convert(Array{Int64,1}, x)
        
        @printf("-> Number of buses: %d \n\n", N)
        @printf("-> Number of lines: %d \n\n", L)
        @printf("-> Number of conductor types: %d \n\n", K)
        @printf("-> Binary variables: \n\n")
        print_2Dtable(convert(Array{Int64,2}, alpha), "Alpha" , "-", "int") # 2D dimensions
        print_table(convert(Array{Int64,1}, x), "x", "-", "int")
        print_table(convert(Array{Int64,1}, beta), "Beta", "-", "int")
        @printf("\n-> Continuous variables: \n\n")
        print_table(I .* BASE_CURRENT, "I" , "A", "float")
        print_table(V .* BASE_VOLTAGE, "V", "kV", "complex")
        print_table(P_s .* BASE_POWER, "P_s", "kW", "float")
        print_table(Q_s .* BASE_POWER, "Q_s", "kVar", "float")
        print_2Dtable(P_conductor .* convert(Array{Int64,2}, alpha) .* BASE_POWER, "P_conductor", "kW", "float")
        print_2Dtable(Q_conductor .* convert(Array{Int64,2}, alpha) .* BASE_POWER, "Q_conductor", "kVar", "float")
