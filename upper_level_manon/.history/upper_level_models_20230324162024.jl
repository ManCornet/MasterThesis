...
function time_dependent_formulation(sets, costs, substation_param, conductor_param, 
    ...
    @variable(model, I_squared[Y, T, K, L])       # Squared current
    @variable(model, P_G[Y, T, N] >= 0)           # Active power generated
    @variable(model, Q_G[N, Y, T])                # Reactive power generated
    @variable(model, S_G[N, Y, T])                # Apparent power generated
    @variable(model, P_cond_forward[K, L, Y, T])  # Forward direction active power flow
    @variable(model, P_cond_backward[K, L, Y, T]) # Backward direction active power flow
    @variable(model, Q_cond_forward[K, L, Y, T])  # Forward direction reactive power flow
    @variable(model, Q_cond_backward[K, L, Y, T]) # Backward direction reactive power flow
    @variable(model, MIN_VOLTAGE^2 <= V_squared[N, Y, T] <= MAX_VOLTAGE^2)
    @variable(model, x[L], Bin)
    @variable(model, X_i_ij[K, N, L, Y, T] >= 0)
    @variable(model, X_ij_re[K, L, Y, T] >= 0)
    @variable(model, X_ij_im[K, L, Y, T])
    @variable(model, alpha[K, L], Bin)
    ...
    for l in L, k in K, i in N, y in Y, t in T 
        if i in line_ends[l]
            continue
        end
        fix(X_i_ij[k, i, l, y, t], 0.0; force=true)
    end

    # ========================== Objective function =============================
    

    @objective(model, Min,  sum(alpha[k, l] * line_cost[k][l] * line_length[l] 
                                for k in K, l in L) 
    ...
    # CONSTRAINT (3)
    @constraint(model, line_constructed[l=L], x[l] == sum(alpha[k, l] for k in K))

    # CONSTRAINT (4)
   
    @constraint(model, 
                substation_capacity_limit[i=Ns, y=Y, t=T], 
                [S_G[i, y, t], P_G[i, y, t], Q_G[i, y, t]] in SecondOrderCone()
                )

    @constraint(model, 
                substation_capacity_limit[i=Ns, y=Y, t=T], 
                S_G[i, y, t] <= S_rating_init[i] + S_allocated[i]
                )
    ...
    # CONSTRAINT (18)
    @constraint(model, 
                active_balance[i=N, y=Y, t=T], 
                P_G[i, y, t] - P_D[i, y, t] == sum(P_cond_forward[k, l, y, t]  
                                                 for k in K, l in Omega_sending[i])
                                                 + sum(P_cond_backward[k, l, y, t] 
                                                 for k in K, l in Omega_receiving[i])
                )

    # CONSTRAINT (19)
    @constraint(model, 
                reactive_balance[i=N, y=Y, t=T], 
                Q_G[i, y, t] - Q_D[i, y, t] == sum(Q_cond_forward[k, l, y, t]
                                                for k in K, l in Omega_sending[i])
                                                + sum(Q_cond_backward[k, l, y, t]
                                                for k in K, l in Omega_receiving[i])
                )

    # CONSTRAINT (20)
    @constraint(model, 
                current_limit[k=K, l=L, y=Y, t=T], 
                I_squared[k, l, y, t] <= alpha[k, l] * max_current[k][l]^2
                )
