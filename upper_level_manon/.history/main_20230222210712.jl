...
function main()
    ...
    if formulation == 1
        ...
        V = [(v_real*BASE_VOLTAGE  + im*v_image*BASE_VOLTAGE) for (v_real, v_imag) in zip(V_re, V_im)]
        ...
        print_table(alpha, "Alpha" , "-") # 2D dimensions
        print_table(x, "x", "-")
        print_table(beta, "Beta", "-")
        @printf("-> Continuous variables: \n")
        print_table(I, "I" , "A")
        print_table(V, "V", "kV")
        print_table(P_s, "P_s", "kW")
        print_table(Q_s, "Q_s", "kVar")
        print_table(P_conductor, "P_conductor", "kW")
        print_table(Q_conductor, "Q_conductor", "kVar")
