...
function time_dependent_formulation(sets, costs, substation_param, conductor_param, 
    ...
    for k in K, l in L, y in Y, t in T
        ...
        # CONSTRAINT (24)
        @constraint(model, MIN_VOLTAGE^2 * alpha[k, l] <= X_i_ij[y, t, l, k, ifrom])
        @constraint(model, MAX_VOLTAGE^2 * alpha[k, l] >= X_i_ij[y, t, l, k, ifrom])
        @constraint(model, MIN_VOLTAGE^2 * alpha[k, l] <= X_i_ij[y, t, l, k, ito])
        @constraint(model, MAX_VOLTAGE^2 * alpha[k, l] >= X_i_ij[y, t, l, k, ito])
