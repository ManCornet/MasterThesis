...
function MISOCP_formulation(N, n_s, n_s_init, L, K ,
   ...
   @objective(model, Min,  K_l              * sum((alpha[k, l] * line_cost[k][l] 
    ...
    @constraint(model, 
                ...
                P_s[i] - P_demand[i] ==   sum(P_conductor[k, l] + active_losses[k, l]
                                            for k in 1:K, l in Omega_sending[i])
                                        - sum(P_conductor[k, l] + active_losses[k, l]
                                            for k in 1:K, l in Omega_receiving[i])
                )
    # CONSTRAINT (19)
    @constraint(model, 
                reactive_balance[i=1:N], 
                Q_s[i] - Q_demand[i] ==   sum(Q_conductor[k, l] + reactive_losses[k, l]
                                            for k in 1:K, l in Omega_sending[i])
                                        - sum(Q_conductor[k, l] + reactive_losses[k, l]
                                            for k in 1:K, l in Omega_receiving[i])
                )
