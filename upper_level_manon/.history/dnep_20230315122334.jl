...
@objective(model, Min, K_l * sum(alpha[k, l] * line_cost[k][l] * line_length[l] for k in 1:K, l in 1:L)
                       ...
                       + HOURS_IN_A_YEAR * (1 + interest_rate_losses) * line_loss * sum((P_s[i] - P_demand[i]) for i in 1:N)
                       + HOURS_IN_A_YEAR * (1 + interest_rate_substation) * substation_utilization * sum((P_s[i]^2 + Q_s[i]^2)  for i in 1:n_s)) 
