...
function MISOCP_formulation(N, n_s, n_s_init, L, K ,
    ...
    @variable(model, P_cond_forward[1:K, 1:L] >= 0)
    @variable(model, P_cond_backward[1:K, 1:L] >= 0)
    @variable(model, Q_cond_forward[1:K, 1:L] >= 0)
    @variable(model, Q_cond_backward[1:K, 1:L] >= 0)
