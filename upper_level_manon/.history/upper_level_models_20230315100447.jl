...
function MISOCP_formulation(N, n_s, n_s_init, L, K ,
    ...
    @variable(model, P_cond_forward[1:K, 1:L])
    @variable(model, P_cond_backward[1:K, 1:L])
    @variable(model, Q_cond_forward[1:K, 1:L])
    @variable(model, Q_cond_backward[1:K, 1:L])
   ...
   @objective(model, Min,  K_l               * sum((alpha[k, l] * line_cost[k][l] 
                          ...
                          + (HOURS_IN_A_YEAR * (1 + interest_rate_losses) 
                                             ...
                                             * sum((P_s[i] - P_demand[i]) for i in 1:N) * BASE_POWER)
                          +  epigraph
                )
    ...
    for k in 1:K, l in 1:L
        ...
        # CONSTRAINT (21)
        @constraint(model, 
                    P_cond_forward[k,l] ==    conductance[k][l] 
        ...
        # CONSTRAINT (21)
        @constraint(model, 
                    P_cond_backward[k,l] ==    conductance[k][l] 
         ...
         # CONSTRAINT (21)
         @constraint(model, 
         Q_cond_forward[k,l] ==    conductance[k][l] 
                                 * (X_i_ij[k, ifrom, l] - X_ij_re[k, l]) 
                                 - susceptance[k][l] * X_ij_im[k, l]
        )
        ...
        # CONSTRAINT (21)
        @constraint(model, 
        Q_cond_backward[k,l] ==    conductance[k][l] 
                                * (X_i_ij[k, ifrom, l] - X_ij_re[k, l]) 
                                - susceptance[k][l] * X_ij_im[k, l]
        )
