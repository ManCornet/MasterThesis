...
function time_dependent_formulation(sets, costs, substation_param, conductor_param, 
    ...
    @variable(model, I_squared[1:Y_size, 1:T_size, 1:L_size, 1:K_size])       # Squared current
    @variable(model, P_G[1:Y_size, 1:T_size, 1:N_size] >= 0)           # Active power generated
    @variable(model, Q_G[1:Y_size, 1:T_size, 1:N_size])                # Reactive power generated
    @variable(model, S_G[1:Y_size, 1:T_size, 1:N_size])                # Apparent power generated
    @variable(model, P_cond_forward[1:Y_size, 1:T_size, 1:L_size, 1:K_size])  # Forward direction active power flow
    @variable(model, P_cond_backward[1:Y_size, 1:T_size, 1:L_size, 1:K_size]) # Backward direction active power flow
    @variable(model, Q_cond_forward[1:Y_size, 1:T_size, 1:L_size, 1:K_size])  # Forward direction reactive power flow
    @variable(model, Q_cond_backward[1:Y_size, 1:T_size, 1:L_size, 1:K_size]) # Backward direction reactive power flow
    @variable(model, MIN_VOLTAGE^2 <= V_squared[1:Y_size, 1:T_size, 1:N_size] <= MAX_VOLTAGE^2)
    @variable(model, x[1:L_size], Bin)
    @variable(model, X_i_ij[1:Y_size, 1:T_size, 1:L_size, 1:K_size, 1:N_size] >= 0)
    @variable(model, X_ij_re[1:Y_size, 1:T_size, 1:L_size, 1:K_size] >= 0)
    @variable(model, X_ij_im[1:Y_size, 1:T_size, 1:L_size, 1:K_size])
    @variable(model, alpha[1:L_size, 1:K_size], Bin)
    @variable(model, beta[1:Ns_size], Bin)
    @variable(model, S_allocated[1:Ns_size] >= 0) 
    @variable(model, active_losses[1:Y_size, 1:T_size] >= 0) 
