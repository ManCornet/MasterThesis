...
function process_conductors(conductor_idx, line_idx)
    max_i = df_conductor.max_i_ka[conductor_idx]
...
S_rating_init = 20*ones(n_s) / BASE_POWER
S_rating_max = 200*ones(n_s) / BASE_POWER
substation_cost = 1e5*ones(n_s)  # EUR


df_load = DataFrames.DataFrame(XLSX.readtable(XLSX_FILE_PATH, "load"))
P_demand = [zeros(n_s); df_load.p_mw / BASE_POWER]
Q_demand = [zeros(n_s); df_load.q_mvar / BASE_POWER]
